...
class ReconstructionRunner:
    ...
    def run(self):
        ...
        self.data = self.data # WT: change from gpu to cpu
        ...
        for epoch in range(self.startepoch, self.nepochs + 1):
            ...
            # WT: debug: change type of indices to torch.long
            ...
            # WT: add outer loss, which is the sum of absolute distance of all data points outside the surface
            # outer loss
            outer_pred=mnfld_pred[mnfld_pred>0]
            mnfld_outer_loss=(outer_pred.abs()).sum()


            # eikonal loss

            grad_loss = ((nonmnfld_grad.norm(2, dim=-1) - 1) ** 2).mean()
            # WT: only include 100 max grad norm into the training loss after epoch_th epochs
            epoch_th=100000
            temp_grad_loss=((nonmnfld_grad.norm(2, dim=-1) - 1) ** 2)
            indices = torch.argsort(temp_grad_loss)[-100: ]
            grad_loss_max100=temp_grad_loss[indices].mean()

            # WT: include epoch threshold and outer loss with coefficient coeff_outer
            #loss = mnfld_loss + self.grad_lambda * grad_loss
            coeff_outer=1/2000*10
            if epoch <epoch_th:
                if torch.isnan(mnfld_outer_loss):
                    loss = mnfld_loss + self.grad_lambda * grad_loss
                else:
                    loss = mnfld_loss + mnfld_outer_loss * coeff_outer + self.grad_lambda * grad_loss
            else:
                if torch.isnan(mnfld_outer_loss):
                    loss = mnfld_loss + self.grad_lambda * grad_loss_max100*5
                else:
                    loss = mnfld_loss + mnfld_outer_loss * coeff_outer + self.grad_lambda * grad_loss_max100*5
            ...
            if epoch % self.conf.get_int('train.status_frequency') == 0:
                # WT: add outer loss output
                if epoch <epoch_th:
                    print('Train Epoch: [{}/{} ({:.0f}%)]\tTrain Loss: {:.6f}\tManifold loss: {:.6f}'
                        '\tGrad loss: {:.6f}\tNormals Loss: {:.6f}\tOuter Loss: {:.6f}'.format(
                        epoch, self.nepochs, 100. * epoch / self.nepochs,
                        loss.item(), mnfld_loss.item(), grad_loss.item(), normals_loss.item(), mnfld_outer_loss.item()))
                else:
                    print('Train Epoch: [{}/{} ({:.0f}%)]\tTrain Loss: {:.6f}\tManifold loss: {:.6f}'
                          '\tGrad loss: {:.6f}\tNormals Loss: {:.6f}\tOuter Loss: {:.6f}'.format(
                        epoch, self.nepochs, 100. * epoch / self.nepochs,
                        loss.item(), mnfld_loss.item(), grad_loss_max100.item(), normals_loss.item(),
                        mnfld_outer_loss.item()))
    ...
    def __init__(self, **kwargs):
        ...
        self.local_sigma = torch.from_numpy(sigmas).float() # WT: change from GPU to CPU
...
if __name__ == '__main__':
    ...
    parser.add_argument('--points_batch', type=int, default=2000, help='point batch size')

...
def plot_threed_scatter(points,path,epoch,in_epoch):
    ...
    layout = go.Layout(width=1200, height=1200, scene=dict(xaxis=dict(range=[-20, 20], autorange=False),
                                                           yaxis=dict(range=[-20, 20], autorange=False),
                                                           zaxis=dict(range=[-20, 20], autorange=False),
...
def plot_surface(decoder,path,epoch, shapename,resolution,mc_value,is_uniform_grid,verbose,save_html,save_ply,overwrite, points=None, with_points=False, latent=None, connected=False):
    ...
    if (not os.path.exists(filename) or overwrite):
        ...
        layout = go.Layout(title= go.layout.Title(text=shapename), width=1200, height=1200, scene=dict(xaxis=dict(range=[-20, 20], autorange=False),
                                                               yaxis=dict(range=[-20, 20], autorange=False),
                                                               zaxis=dict(range=[-20, 20], autorange=False),
                                                               aspectratio=dict(x=1, y=1, z=1)))
        if (with_points):
    ...
    if (not (np.min(z) > mc_value or np.max(z) < mc_value)):
        ...
        if (save_ply):
            ...
            if connected:
                ...
                meshexport = max_comp
            #tt
            meshexport.export('U:/Documents/IGR-master/exps/single_shape/mesh_0.stl')
            # tt compare mean and max Hausdorff Distance
            import pymeshlab
            mesh_set = pymeshlab.MeshSet()
            mesh_set.load_new_mesh('U:/Documents/IGR-master/compare/66900_sca_R_tfm_rot.stl')
            mesh_set.load_new_mesh('U:/Documents/IGR-master/exps/single_shape/mesh_0.stl')
            ant = mesh_set.apply_filter('hausdorff_distance', targetmesh=1, sampledmesh=0, savesample=True)
            max_HD = ant['max']
            mean_HD = ant['mean']
            meshexport.export('U:/Documents/IGR-master/exps/single_shape/mesh_{0:.6f}_{1:.6f}.stl'.format(max_HD,mean_HD))
